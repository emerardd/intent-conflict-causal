...
def source_files(root: Path) -> list[Path]:
    ...
    files += list((root / "configs").glob("stage3*.json"))
    files += list((root / "docs").glob("stage3*preregistration.md"))
...
def inference(config: dict, config_path: Path, phase: str, root: Path,
              builder=build_trials, renderer=render_trial) -> Path:
    ...
    trials = builder(phase, config["seed"])
    ...
    for t in trials:
        ids, pos = tokenize_trial(loaded.tokenizer, t, messages=renderer(t))
    ...
    token_records = [{"example_id": t.example_id, "ids": ids, "positions": pos,
        ...
        "messages": renderer(t)} for t, ids, pos in zip(trials, prompt_ids, positions, strict=True)]
...
def intervene(output: Path, root: Path) -> dict:
    ...
    if not config.get("include_action_control", True):
        del axes["action"]
