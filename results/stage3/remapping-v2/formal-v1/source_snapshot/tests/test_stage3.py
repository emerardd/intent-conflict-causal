...
def test_remap_v2_has_fresh_pilots_and_only_two_factors():
    from intent_conflict.stage3_remap import build_remap, render_remap
    screen, formal = build_remap("screen"), build_remap("formal")
    assert len(screen) == 16 and len(formal) == 288
    assert not {r.scenario_id for r in screen} & {r.scenario_id for r in build_trials("screen")}
    for t in screen+formal:
        assert not t.reversed_policy
        text = render_remap(t)[1]["content"]
        assert "REVERSED controller" not in text and "NORMAL controller" not in text
        assert "A =" not in text.split("[RULE_END]")[0]
    assert [sum(t.split == split for t in formal) for split in ("train", "validation", "test")] == [64, 32, 192]


def test_remap_metrics_and_two_cell_behavior_are_finite():
    from intent_conflict.stage3_remap import build_remap, site_metrics
    rows = []
    for t in build_remap("formal"):
        rows.append(dict(t.row(), top1_id=32 if t.expected_label == "A" else 33,
            top1_label=t.expected_label, ab_label=t.expected_label, ab_mass=1.,
            generated_ids=[32 if t.expected_label == "A" else 33], generated_text=t.expected_label))
    x = np.stack([target(rows, n).astype(float)*2-1 for n in ("auth", "token")], 1)
    # Within one mapping the two labels coincide. Give the invariant auth axis
    # higher amplitude so the raw cross-mapping contrast also transfers.
    x[:, 0] *= 2
    metrics, dirs = site_metrics(rows, x)
    assert metrics["selection_score"] == 1
    np.testing.assert_allclose(dirs["auth_clean"], [1, 0], atol=1e-12)
    behavior = behavior_summary(rows)
    assert len(behavior["test/seen"]["cells"]) == 2
    assert behavior["test/seen"]["min_cell_top1_accuracy"] == 1


def test_independent_truth_table_matches_all_frozen_trials():
    from intent_conflict.stage3_audit import expected_answer
    for trial in build_trials("screen"):
        block, label = expected_answer(trial.row())
        assert block == trial.assigned_block and label == trial.expected_label
