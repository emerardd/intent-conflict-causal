...
def summarize_interventions(rows: list[dict], baseline: dict[str, dict], seed: int, samples: int) -> dict:
    ...
    for key, pairs in effects.items():
        ...
        cells = {f"policy{int(p)}_map{int(m)}": float(np.mean([v for eid, v in pairs if baseline[eid]["reversed_policy"] == p and baseline[eid]["reversed_mapping"] == m])) for p in (False, True) for m in (False, True)}
